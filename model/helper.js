...
export const checkJson = (target, onlyCheck = false) => {
  try {
  ...
  return {
    set(name, schema) {
      this.hasOwnProperty(name) &&
        console.log(`${horizonBar} Overwrited model: ${name}. ${horizonBar}`);
...
export const createAuthor = (() => {
  const keys = ["create_id", "create_name", "modify_id", "modify_name"];
  return (author) =>
    keys.reduce((dict, key) => ({ ...dict, [key]: author }), {});
})();
...
/** in development */
const initSequelizeModel = async (req, res, next) => {
  const sequelize = await connection.get();
  if (!sequelize) return process;
